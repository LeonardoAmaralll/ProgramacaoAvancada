...
///////////////////////////////////////////////////////////////////////////////////
  ...
  do{
    ...
    if(atual.pos != dest)
    {
        for(int i = -1; i <= 1; i++)
        {
            for(int j = -1; j <= 1; j++)
            {
                if(dir(int(i),int(j)) != Coord(0,0))
                {
                    prox = atual.pos + dir(int(i),int(j));
                    ...
                    if(movimentoValido(atual.pos,prox))
                    {
                        ...
                        if(oldF == Fechado.end() && oldA == Aberto.end())
                        {
                            ...
                            big = find_if(Aberto.begin(),Aberto.end(),Maior(suc));
                            Aberto.insert(big,suc);
                            cout << "[" << suc.pos << "|" << suc.ant << "|" << suc.g << "|" << suc.h << "|" <<suc.f << "]" << endl;
                        }
                    }
                }
            }
  ...
  else
  {
      ...
      {
          set(atual.pos,EstadoCel::CAMINHO);
          list<Noh>::iterator k;
...
Noh::Noh(Coord& p,Coord& a,double gg, Coord& dest)
{
    pos = p;
    ant = a;
    g = gg;
...
double Noh::CustoPas(const Noh& A)
{
    if((pos.lin == ant.lin || pos.col == ant.col) && (pos != ant)) return (A.g + 1.0);
...
bool Noh::operator==(const Coord& p)
{
    return (pos == p);
}

Noh Noh::operator=(Noh& A)
{
    Noh a;
    a.pos = A.pos;
    a.ant = A.ant;
...
//////////////////Classe de busca//////////////////////////////////////
Maior::Maior(const Maior& P): A(P.A)
{}

inline Maior::Maior(Noh& a): A(a)
{}

bool Maior::operator()(const Noh& B) const
{
