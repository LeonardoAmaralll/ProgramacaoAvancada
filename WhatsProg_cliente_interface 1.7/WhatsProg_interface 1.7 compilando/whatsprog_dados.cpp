...
testRemetStatus::testRemetStatus(std::string R, MsgStatus S): remetente(R), status(S) {}

/// Testar mensagens com um dado remetente e id
testRemetId::testRemetId(std::string R, int32_t I): remetente(R), id(I) {}

/// Testar mensagens com um dado destinatario e status
testDestStatus::testDestStatus(std::string D, MsgStatus S): destinatario(D), status(S) {}

/// Testar mensagens com um dado destinatario e id
testDestId::testDestId(std::string D, int32_t I): destinatario(D), id(I) {}
...
const std::string& Mensagem::getRemetente() const
{
  return remetente;
}

bool Mensagem::setRemetente(const std::string& R)
{
...
const std::string& Mensagem::getDestinatario() const
{
  return destinatario;
}

bool Mensagem::setDestinatario(const std::string& D)
{
...
const std::string& Mensagem::getTexto() const
{
  return texto;
}

bool Mensagem::setTexto(const std::string& T)
{
...
bool Mensagem::operator==(const testRemetStatus& T) const
{
  return remetente==T.remetente && status==T.status;
}

bool Mensagem::operator==(const testRemetId& T) const
{
  return remetente==T.remetente && id==T.id;
}

bool Mensagem::operator==(const testDestStatus& T) const
{
  return destinatario==T.destinatario && status==T.status;
}

bool Mensagem::operator==(const testDestId& T) const
{
...
bool Mensagem::ler(std::ifstream& arq)
{
  int32_t prov_id;
  int prov_status;
  std::string prov_remetente;
  std::string prov_destinatario;
  std::string prov_texto;
...
bool Mensagem::salvar(std::ofstream& arq) const
{
  if (!arq.is_open()) return false;
  arq << id << ' ' << (int)status << ' '
