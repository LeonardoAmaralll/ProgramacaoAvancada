...
const std::string& Conversa::getCorrespondente() const {return correspondente;}

bool Conversa::setCorrespondente(const std::string& U)
{
...
bool Conversa::operator==(const std::string& C) const {return correspondente==C;}
...
{
  std::vector<Mensagem>::const_iterator iM;
...
bool Conversa::ler(std::ifstream& arq)
{
  std::string prov_cabecalho;
  std::string prov_correspondente;
  int prov_size;
...
/// (da classe DadosCliente)
bool Conversa::salvar(std::ofstream& arq) const
{
  if (!arq.is_open()) return false;
  arq << "CONVERSA " << correspondente << ' ' << LM.size() << "\n";
...
const std::string& WhatsProgDadosCliente::getServidor() const {return servidor;}
const std::string& WhatsProgDadosCliente::getMeuUsuario() const {return meuUsuario;}

bool WhatsProgDadosCliente::setServidorUsuario(const std::string& IP, const std::string& MU)
{
  mtx.lock();
...
bool WhatsProgDadosCliente::setConversaAtual(const std::string& U)
{
  if (!testarNomeUsuario(U)) return false;
...
/// Retorna false em caso de erro, true se OK
bool WhatsProgDadosCliente::insertConversa(const std::string& U)
{
...
IterConversa WhatsProgDadosCliente::findConversa(const std::string& U)
{
  IterConversa it=find(LC.begin(), LC.end(), U);
  return it;
}
...
std::string WhatsProgDadosCliente::getNomeArq() const
{
...
bool WhatsProgDadosCliente::ler()
{
  ...
  std::ifstream arq(getNomeArq().c_str());
  if (!arq.is_open())
  {
    mtx.unlock();
    return false;
  ...
  std::string prov_cabecalho;
  std::string prov_servidor;
  std::string prov_meuUsuario;
  int prov_idMensagem;
  int prov_size;
...
bool WhatsProgDadosCliente::salvar()
{
  ...
  // Se nao conseguir abrir o arquivo, encerra
  std::ofstream arq(getNomeArq().c_str());
  if (!arq.is_open())
  {
    mtx.unlock();
    return false;
